...
const emptySetting = {
  status: "active",
  sectionTitle: { bn: "ক্রিকেক্স সুবিধাসমূহ", en: "CRICKEX ADVANTAGE" },
  cards: [],
...
const AffiliateAdvantageSetting = () => {
  ...
  const loadSetting = async () => {
    try {
      setPageLoading(true);
      const res = await api.get("/api/affiliate-advantage-settings");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append("cardBg", form.cardBg || "#e8f8ff");
      fd.append("cardTitleColor", form.cardTitleColor || "#002d68");
      fd.append("cardDescColor", form.cardDescColor || "#001d55");

      fd.append("contentMaxWidth", form.contentMaxWidth || "1425px");
      fd.append("iconSize", form.iconSize || "52px");
      ...
      await api.put("/api/affiliate-advantage-settings", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      toast.success("Affiliate advantage setting saved successfully");
      await loadSetting();
    } catch (error) {
  ...
  const handleRemoveIcon = async (parentId) => {
    ...
    try {
      setLoading(true);
      await api.patch("/api/affiliate-advantage-settings/remove-icon", {
        parentId,
      });
  ...
  const handleResetColors = async () => {
    ...
    try {
      setLoading(true);
      await api.patch("/api/affiliate-advantage-settings/reset-colors");
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Control advantage cards, icons, title, colors and layout.
            </p>
          </div>
      ...
      >
          ...
          <Section title="Title, Layout & Status">
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <SmallInput
                label="Icon Size"
                value={form.iconSize}
                onChange={(v) => setForm({ ...form, iconSize: v })}
              />
            </div>
          </Section>

          <ListSection title="Advantage Cards" onAdd={addCard}>
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        ...
        <aside className="sticky top-6 h-fit space-y-6">
          <Section title="Quick Preview">
            <AdvantagePreview form={form} small />
          </Section>
...
const AdvantagePreview = ({ form, small = false }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="mb-8 rounded-md px-4 py-5 text-center shadow-lg"
        ...
        >
          ...
          >
            {getText(form.sectionTitle, "CRICKEX ADVANTAGE")}
          </h2>
        </div>

        <div
          className={`grid grid-cols-2 gap-6 sm:gap-8 ${
            small ? "" : "md:grid-cols-2 lg:grid-cols-4"
          }`}
        >
          {cards.length ? (
            cards.map((card, index) => {
              ...
              return (
                <div
                  ...
                  style={{ backgroundColor: form.cardBg || "#e8f8ff" }}
                >
                  {icon && (
                    <img
                      src={icon}
                      alt={getText(card.title, "Advantage")}
                      className="mb-6 object-contain"
                      style={{
                        width: form.iconSize || "52px",
                        height: form.iconSize || "52px",
                      }}
                      draggable={false}
                    />
                  )}
                  ...
                  >
                    {getText(card.title, "CARD TITLE")}
                  </h3>

                  <p
                    className="max-w-[230px] text-[12px] font-medium leading-[1.35] sm:text-[15px]"
                    style={{ color: form.cardDescColor || "#001d55" }}
                  >
                    {getText(card.description, "Card description")}
                  </p>
                </div>
              );
            })
