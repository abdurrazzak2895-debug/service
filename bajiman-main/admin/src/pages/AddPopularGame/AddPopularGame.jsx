...
const AddPopularGame = () => {
  ...
  const loadGames = async () => {
    try {
      ...
      const res = await api.get("/api/popular-games", {
        params: {
          search,
          status: statusFilter,
          limit: 100,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing?._id) {
        await api.put(`/api/popular-games/${editing._id}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });

        toast.success("Popular game updated successfully");
      } else {
        await api.post("/api/popular-games", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleDelete = async (id) => {
    const ok = window.confirm(
      "Are you sure you want to delete this popular game?",
    );
    if (!ok) return;

    try {
      await api.delete(`/api/popular-games/${id}`);
  ...
  const handleRemoveImage = async () => {
    ...
    try {
      await api.patch(`/api/popular-games/${editing._id}/remove-image`);
    ...
    <div className="space-y-6 text-white">
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Add popular game by Game ID, landscape picture, order and status.
            </p>
          </div>
          ...
          <div className="mb-6 flex items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-black">
                {editing ? "Update Popular Game" : "Create Popular Game"}
              </h2>
              <p className="text-sm text-slate-400">
                Game name নেওয়া হবে না, শুধু Game ID এবং landscape image.
              </p>
            </div>
      ...
      <section className="rounded-3xl border border-[#1A79D3]/20 bg-black/35 p-5 shadow-2xl md:p-6">
        <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            <h2 className="text-xl font-black">Popular Game List</h2>
            <p className="text-sm text-slate-400">
              Total {games.length} popular games found
            </p>
          </div>
